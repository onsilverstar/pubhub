...
public class ViewBookDetailsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String tagremove=request.getParameter("removetag");
		System.out.println(request.getParameter("removetag"));
		BookDAO dao = DAOUtilities.getBookDAO();
		TagDAO tagdao=DAOUtilities.getTagDAO();
		Book book = dao.getBookByISBN(isbn13);
		List<String> tags=tagdao.getTags(book);
		if(tagname!="")
		{
			tagdao.addTag(book, request.getParameter("tagname"));
		}
		if(tagremove!=null)
		{
			tags.forEach(currtag->{tagdao.removeTag(book, currtag);});
		}
